...
<div class="notification-layout">
    ...
    <aside class="notification-sidebar">
        <?php if ($role === "admin" || $role === "principal" || $role === "teacher"): ?>
            <a href="/notifications/create" class="compose-btn">➕ New Notification</a>
        <?php endif; ?>
    ...
    <section id="notification-list" class="notification-list">
        ...
        <?php if (empty($notifications)): ?>
            <div class="empty">No more notifications available.</div>
        <?php else: ?>
            <div class="bulk-bar">
                <label>
            ...
            <?php foreach ($notifications as $n): ?>
                <div class="notification-item <?= $n['is_read'] ? 'read' : 'unread' ?>"
                    ...
                    <!-- Listing all required notifications -->
    ...
    document.addEventListener('click', function (e) {
        ...
        if (e.target.id === 'bulk-delete') {
            const ids = [...document.querySelectorAll('.select-item:checked')]
                .map(cb => cb.value);
            if (!ids.length) return;
            fetch('/notifications/bulkDelete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ids })
            })
